...
class Turbo::Streams::TagBuilder
  ...
  # Replace the <tt>target</tt> in the dom with the either the <tt>content</tt> passed in or a rendering result determined
  ...
  def action(name, target, content = nil, **rendering, &block)
    case
    when content
      turbo_stream_action_tag name, target: target, template: content
    when block_given?
      turbo_stream_action_tag name, target: target, template: @view_context.capture(&block)
    when rendering.any?
      turbo_stream_action_tag name, target: target, template: @view_context.render(formats: [ :html ], **rendering)
    else
      turbo_stream_action_tag name, target: target
    end
  end
end
