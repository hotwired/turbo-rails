...
# <tt>Turbo::StreamChannel</tt>, but each with their own subscription. Since stream names are exposed directly to the user
...
module Turbo::Stream::StreamName
  # Used by <tt>Turbo::StreamChannel</tt> to verify a signed stream name.
  def verified_stream_name(signed_stream_name)
    Turbo.signed_stream_verifier.verified signed_stream_name
  end

  # Used by <tt>Turbo::StreamHelper#subscribe_to_turbo_stream_from_signed(*streamables)</tt> to generate a signed stream name.
