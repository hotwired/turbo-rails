module Turbo
  module TestAssertions
    ...
    def assert_page_update(command:, element: nil, container: nil, &block)
      assert_response :ok
      assert_equal Mime[:page_update], response.media_type
      assert_select %(template[data-page-update="#{command}##{element ? dom_id(element) : container}"]), count: 1, &block
    end

    def assert_no_page_update(command:, element: nil, container: nil)
      assert_response :ok
      assert_equal Mime[:page_update], response.media_type
